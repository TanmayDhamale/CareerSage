...
# One-Hot Encode categorical features (Gender, Education, Skills, Interests)
X = pd.get_dummies(X)
...
# Ensure X_train and X_test are converted to float32
scaler = StandardScaler()
X_train = scaler.fit_transform(X_train).astype(np.float32)
X_test = scaler.transform(X_test).astype(np.float32)

# Ensure y_train and y_test are converted to int32
